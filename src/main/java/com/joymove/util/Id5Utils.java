...
import cn.id5.gboss.businesses.validator.service.app.QuerySingle;
import cn.id5.gboss.businesses.validator.service.app.QueryValidatorServices;
import cn.id5.gboss.businesses.validator.service.app.QueryValidatorServicesService;
...
public class Id5Utils {
    ...
    public  static boolean checkStatusElement(Element target,int wantStatus) {
        Element messageNode = getDirectChild(target,"message");
        Element statusNode = getDirectChild(messageNode,"status");
        if(Integer.parseInt(statusNode.getTextContent())==wantStatus) {
            return true;
        } else
            return false;
    }
    ...
    public static Map<String,String> checkXmlResult(String xml) throws  Exception {
        ...
        Document doc = db.parse(new InputSource(new ByteArrayInputStream(xml.getBytes("utf8"))));

        NodeList nodeList = doc.getElementsByTagName("data");
        Element  dataNode = (Element)nodeList.item(0);
        Element policeCINode = (Element)dataNode.getElementsByTagName("policeCheckInfos").item(0);
        policeCINode = getDirectChild(policeCINode,"policeCheckInfo");
        Integer compStatus = Integer.parseInt(getDirectChild(policeCINode, "compStatus").getTextContent());

        if(checkStatusElement(dataNode,0) && checkStatusElement(policeCINode,0) && compStatus==3) {
            infoMap = new HashMap<String,String>();
            String sex2 = getDirectChild(policeCINode, "sex2").getTextContent();
            String idNo = getDirectChild(policeCINode, "identitycard").getTextContent();
            String idName = getDirectChild(policeCINode, "name").getTextContent();
